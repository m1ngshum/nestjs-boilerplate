...
async function bootstrap() {
  ...
  // Swagger UI bundles use eval() internally; widen scriptSrc only when Swagger
  // is enabled and this is not a production deployment.
  const isProductionDeployment = process.env.ENVIRONMENT === 'production';
  const scriptSrc =
    configService.swagger.enabled && !isProductionDeployment
      ? Array.from(new Set([...securityConfig.csp.directives.scriptSrc, `'unsafe-eval'`]))
  ...
  if (configService.swagger.enabled) {
    ...
    // @fastify/compress corrupts dynamically-generated responses (Content-Encoding: gzip
    // with Content-Length: 0). Only the paths below are dynamically generated by NestJS —
    // static assets (swagger-ui-bundle.js, swagger-ui.css, …) keep compression.
